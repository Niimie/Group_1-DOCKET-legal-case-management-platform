...
function goToFill() {
    ...
    sectionTitle.forEach((sectionKeyword) => sectionKeyword.classList.remove("active"))
    sectionTitle[currentSlide].classList.add("active")
    progressBars.forEach(progressBar => progressBar.classList.remove("current"))
...
nextBtns.forEach((nextBtn) => {
    nextBtn.addEventListener("click", function () {
        firmPlaces.forEach((firmPlace) => firmPlace.innerHTML = `${lawFirm.value}`);
        changeSection(currentSlide + 1);
        sectionTitle.forEach((sectionKeyword) => sectionKeyword.classList.remove("active"));
        sectionTitle[currentSlide].classList.add("active");
        progressBars[currentSlide].classList.add("current");
...
form.addEventListener("submit", function (e) {
    ...
    if (!workEmail.value.includes('@')) {
        goToFill();
        document.getElementById("email-error").textContent = "❗Email address must contain '@' keyword.";
        removeError();
        return false;
    }
    ...
    if (workEmail.value.length < 11) {
        goToFill();
        document.getElementById("email-error").textContent = "❗This is not a valid email address";
        removeError();
        return false;
    }
    ...
    if (lawFirm.value.trim() === "") {
        goToFill();
        document.getElementById("firm-error").textContent = "❗Law firm is required";
        removeError();
        return false;
    }
    ...
    if (selectedRole == null) {
        goToSelect()
        document.getElementById("radio-error").textContent = "❗An option must be selected";
        removeError();
        return false;
    }
    ...
    if (password.value.length < 6) {
        document.getElementById("password-error").textContent = "❗Password must be a minimum of 6 characters";
        removeError();
        return false;
    }

    if (repeatedPassword.value.trim() === "") {
        document.getElementById("repeat-error").textContent = "❗Please confirm your password";
        removeError();
        return false;
    }

    if (repeatedPassword.value !== password.value) {
        document.getElementById("repeat-error").textContent = "❗The passwords do not match";
        removeError();
        return false;
    }
